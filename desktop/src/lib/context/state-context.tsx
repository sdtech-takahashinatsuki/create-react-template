...
export function createStateContext<T>({errorMessage, initialState}:StateContext<T>){
    const {createNg,createOk,isNG} = resultUtility;
    const ContextProvider = createContext<Result<[T,Dispatch<SetStateAction<T>>],Error>>(createNg(new Error(errorMessage)));
