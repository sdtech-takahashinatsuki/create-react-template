...
describe('http-scheme', () => {
  it('createHttpSchemeにおいてhttpErrorStatusResponseの正常な値が返ってくる', () => {
    ...
    expect(httpErrorScheme.httpErrorStatusResponse.internalServerError).toEqual(
      500,
    )
  ...
  it('createHttpSchemeにおいてhttpCustomStatusScheme', () => {
    ...
    expect(httpErrorScheme.httpCustomStatusScheme.notFoundAPIUrl).toEqual(4040)
    expect(httpErrorScheme.httpCustomStatusScheme.returnNotFoundAPIUrl).toEqual(
      4041,
    )
    expect(httpErrorScheme.httpCustomStatusScheme.returnNoPermission).toEqual(
      4030,
    )
    expect(httpErrorScheme.httpCustomStatusScheme.returnBadRequest).toEqual(
      4000,
    )
    expect(httpErrorScheme.httpCustomStatusScheme.schemeError).toEqual(5000)
    expect(httpErrorScheme.httpCustomStatusScheme.serverError).toEqual(5001)
    expect(httpErrorScheme.httpCustomStatusScheme.parseError).toEqual(8888)
    expect(httpErrorScheme.httpCustomStatusScheme.unknownError).toEqual(9999)
  })
  ...
  it('createHttpSchemeにおいてerrorMessage', () => {
    ...
    expect(httpErrorScheme.errorMessage.parseError).toEqual('パースエラーです')
    expect(httpErrorScheme.errorMessage.unknownError).toEqual('unknown error')
  })
})
