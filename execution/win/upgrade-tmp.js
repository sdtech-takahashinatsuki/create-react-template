...
// === 設定 ===
const ZIP_URL = "https://github.com/ShionTerunaga/create-react-template/archive/refs/heads/release.zip";
const ZIP_PATH = path_1.default.resolve("repo.zip");
const TEMP_DIR = path_1.default.resolve("repo");
const EXEC_DIR = path_1.default.join(TEMP_DIR, "create-react-template-release", "execution", "win");
// Where tools will be placed on the user's machine
const TOOLS_DIR = path_1.default.join((0, node_os_1.homedir)(), "tools");
// === 関数群 ===
...
async function main() {
    try {
        ...
        try {
            console.log(`🔗 Attempting to run: npm link -g . (cwd=${TOOLS_DIR})`);
            (0, child_process_1.execSync)("npm link -g .", { stdio: "inherit", cwd: TOOLS_DIR });
            console.log("✅ Successfully linked tools globally (npm link -g .)");
        }
        catch (err) {
